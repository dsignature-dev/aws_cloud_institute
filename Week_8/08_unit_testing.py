...
class AccountTest(unittest.TestCase):
    ...
    def test_get_balance(self):
        account = Account(1001, 3)
        account.withdraw(2)
        self.assertEqual(account.get_balance(), 1)


class SavingsAccountTest(unittest.TestCase):

    def test_constructor(self):
        account = SavingsAccount(1001, 0, 0.24)
        self.assertEqual(account.account_number, 1001)
        self.assertEqual(account.balance, 0)
        self.assertEqual(account.interest_rate, 0.24)

    def test_interest_rate_set(self):
        account = SavingsAccount(1, 0, 0.24)
        self.assertEqual(account.interest_rate, 0.24)
