...
while not order_complete:
    ...
    item["container"] = valid_input(
        container_prompt, container_reprompt, validate_container)
    ...
    number_of_scoops = valid_input(
        scoops_prompt, scoops_reprompt, validate_scoops)
    ...
    for i in range(int(number_of_scoops)):
        ...
        scoop_flavor = valid_input(
            flavor_prompt, flavor_reprompt, validate_flavor)
    ...
    continue_order = valid_input(
        continue_prompt, continue_reprompt, validate_yes_no)
...
orders_file = open(f"{orders_file_name}", 'a')

# Convert order to JSON object and write to file
json.dump(items, orders_file)


# Ensure next order is written on a new line
orders_file.write('\n')
